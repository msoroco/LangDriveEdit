...
def load_model_from_config(ckpt, verbose=False):
    ...
    print(ckpt)
    # ckpt = BleachNick/SD3_UltraEdit_w_mask
...
def main():
    ...
    def edit_image(input_path, output_path, edit_instruction, remove_mask_img=None, mask_img=None):
        ...
        if remove_mask_img is not None:
            remove_mask_img = Image.open(remove_mask_img).convert("RGB")
            remove_mask_img = fit_to_resolution(remove_mask_img, args.resolution)

        if mask_img is not None and remove_mask_img is not None:
            combined_mask = Image.fromarray(np.bitwise_or(np.array(mask_img), np.array(remove_mask_img)))
        elif mask_img is not None:
            combined_mask = mask_img
        elif remove_mask_img is not None:
            combined_mask = remove_mask_img
        else:
            combined_mask = None

        if combined_mask is None:
            combined_mask = Image.new("RGB", input_image.size, (255, 255, 255))
        # combined_mask = fit_to_resolution(combined_mask, args.resolution)
