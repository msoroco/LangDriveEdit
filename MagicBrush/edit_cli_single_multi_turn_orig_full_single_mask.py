...
def main():
    ...
    def edit_image(input_path, output_path, edict_instruction, mask_img=None, add_mask_img=None):
        ...
        if mask_img is not None and add_mask_img is not None:
            print("using combined mask")
            mask_array = np.array(mask_img, dtype=np.uint8)
            add_mask_array = np.array(add_mask_img, dtype=np.uint8)
            combined_mask_array = np.bitwise_or(mask_array, add_mask_array)
            combined_mask = Image.fromarray(combined_mask_array)
            # combined_mask = Image.fromarray(np.bitwise_or(np.array(mask_img), np.array(add_mask_img)))
        elif mask_img is not None:
            combined_mask = mask_img
        elif add_mask_img is not None:
            combined_mask = add_mask_img
        else:
            combined_mask = None

        mask_tensor = 2 * torch.tensor(np.array(combined_mask)).float() / 255 - 1
        ...
        with torch.no_grad(), autocast("cuda"), model.ema_scope():
            ...
            if mask_tensor is not None:
                ...
                # Concatenate the encoded input and mask along channel dimension
                concat_cond = torch.cat([encoded_input, encoded_mask], dim=1)
                cond["c_concat"] = [concat_cond]
            else:
                # If no mask provided but model expects 12 channels, create zero mask
                if hasattr(config.model.params, 'unet_config') and config.model.params.unet_config.params.in_channels == 12:
                    # Create a zero mask with same spatial dimensions
                    zero_mask = torch.zeros_like(encoded_input)
                    concat_cond = torch.cat([encoded_input, zero_mask], dim=1)
                    cond["c_concat"] = [concat_cond]
                else:
