...
def main():
    print("full single mask InstructPix2Pix Editing")
    ...
    def edit_image(input_path, output_path, edit_instruction, mask_img=None, add_mask_img=None):
        ...
        # pixel wise or:
        if mask_img is not None and add_mask_image is not None:
            combined_mask = Image.fromarray(np.bitwise_or(np.array(mask_img), np.array(add_mask_image)))
        elif mask_img is not None:
            combined_mask = mask_img
        elif add_mask_image is not None:
            combined_mask = add_mask_image
        else:
            combined_mask = None
